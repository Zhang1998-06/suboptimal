...
class KinematicObservation(ObservationType):
    ...
    def __init__(self, env: 'AbstractEnv',
        ...
        self.absolute = absolute  # true for DS, Flase for training, use relative coordinate for surronding vehicles
        self.order = order
        self.normalize = normalize#  false for DS, not normalize, true for training 
        self.clip = clip
    ...
    def observe(self) -> np.ndarray:
        ...
        if close_vehicles:
            ...
            origin = self.observer_vehicle if not self.absolute else None  #if want absolute it is none,

            df = pd.concat([df, pd.DataFrame.from_records(
                [v.to_dict(origin, observe_intentions=self.observe_intentions)
                 for v in close_vehicles[-self.vehicles_count + 1:]])[self.features]],   # here if you want the ego coordinate, let the orgin because the ego vehicle
                           ignore_index=True)
        # Normalize and clip
    ...
    def observe_rule(self) -> np.ndarray:
        if not self.env.road:
            return np.zeros(self.space().shape)
        ...
        if close_vehicles:
            #print(self.observer_vehicle)
            origin =  None  #if want absolute it is none,
