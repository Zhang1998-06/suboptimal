...
class HSAC():
    ...
    def __init__(self, env, actor_lr, critic_lr,alpha_lr, target_entropy, tau, gamma, device,replay_memory_size=10000, batch_size=128, control_path="low_level_controller.pth"):
        ###############
        # BUILD MODEL #
        ###############
        self.env = env
    ...
    def update_controller(self, offline_replay, online_replay, bc_weight=0.0):
        ...
        if bc_weight > 0.0:
            off_states_var = Variable(torch.from_numpy(off_states).type(dtype)).to(self.device)
            off_actions_var = Variable(torch.from_numpy(off_actions).long()).to(self.device).unsqueeze(1)
            off_probs = self.actor(off_states_var)
            off_log_probs = torch.log(off_probs + 1e-8)
            bc_loss = -torch.mean(off_log_probs.gather(1, off_actions_var))
    ...
    def save_model(self, control_path=None):
        path = control_path or self.control_path
        torch.save({
            'actor': self.actor.state_dict(),
        }, path)


    def load(self, control_path=None):
        path = control_path or self.control_path
