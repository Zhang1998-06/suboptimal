...
class AbstractEnv(gym.Env):
    ...
    def _info(self, obs: Observation, action: Action) -> dict:
        ...
        info = {
            ...
            "action": action,
            "rule_state":self.observation_type.observe_rule()
        }
        try:
    ...
    def reset(self) -> Observation:
        ...
        return self.observation_type.observe(), self.observation_type.observe_rule()
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:
        ...
        env_reward = self._reward(action)  
        terminal = self._is_terminal()
        info = self._info(obs, action)
        if self.vehicle.crashed==True:
            print("crashed")
        return obs, env_reward,terminal, info #  high_level_reward,low_level_reward,

    #if training obs, high_level_reward,low_level_reward, env_reward, terminal, info
    #if visualizing obs, env_reward, terminal, info

    def goal_low_action(self):
        # you input the highlevel goal, get the low-level action of the highlevel goal without execute it.
        return self.action_type.pre_act()
    
    def _simulate(self, action: Optional[Action] = None) -> None:
        """Perform several steps of simulation with constant action."""
        frames = int(self.config["simulation_frequency"] // self.config["policy_frequency"]) 
        # this frame is 7 which means, it act 7 times in a time step  
        # make it 1 thus for pre_low_level and our SAC low_level have the same update rate
        for frame in range(frames):
            ...
            if action is not None \
                ...
                #print(action)
                self.action_type.act(action)  # sinee we did not define act in the goal class, 
                # this goes to descrete action 
            #print()
            self.road.act()
    ...
    def render(self, mode: str = 'human',reward=None,accu_reward=None, goal_1=None) -> Optional[np.ndarray]:
        """
        ...
        self.viewer.display(reward,accu_reward, goal_1)
