...
class EnvViewer(object):
    ...
    def handle_events(self) -> None:
        ...
        '''
        pygame_str =  '-rew:' + 'str(np.round(reward, 3))'
        notice1=pygame.font.Font(None,self.size)
        textrect=notice1.render(pygame_str,True,(255,0,0))

        self.screen.blit(textrect,[10,10])
        '''
    ...
    def display(self,reward=None,accu_reward=None, goal=None) -> None:
        ...
        if not reward==None and not reward==0:
            self.maxreward=reward
        if not accu_reward==None and not accu_reward==0:
            self.accu_reward=accu_reward
        if not goal==None:
            self.current_Goal=goal
        '''无效
        pygame_str =  '-rew:' + str(reward)
        notice1=pygame.font.Font(None,self.size)
        textrect=notice1.render(pygame_str,True,(255,0,0))
        self.sim_surface.blit(textrect,[50,50])
        '''
        RoadGraphics.display(self.env.road, self.sim_surface,reward=self.maxreward,accu_reward=self.accu_reward, current_Goal=self.current_Goal)
