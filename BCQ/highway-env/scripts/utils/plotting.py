...
def plot_episode_stats(stats, smoothing_window=5, mean_number=10, noshow=False):
    ...
    num_full_chunks = len(stats.episode_rewards) // mean_number
    plt.plot(rewards_smoothed,label="smoothed_episode_reward")

    if num_full_chunks:
        trimmed = stats.episode_rewards[: num_full_chunks * mean_number]
        mean_reward = np.average(trimmed.reshape(-1,mean_number),axis=1)
        mean_axis = np.array([i*mean_number for i in range(mean_reward.shape[0])])
        plt.plot(mean_axis,mean_reward,label=f"mean_reward for {mean_number} episodes")
    plt.xlabel("Epsiode")
    plt.ylabel("Epsiode Reward (Smoothed)")
    plt.title("Episode Reward over Time (Smoothed over window size {})".format(smoothing_window))
