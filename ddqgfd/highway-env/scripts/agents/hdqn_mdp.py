...
class DDPGfD:
    ...
    def __init__(self, env, actor_lr, critic_lr, tau, gamma, device, replay_memory_size=10000, batch_size=128):
        self.env = env
        self.batch_size = batch_size
        self.observation_space = env.observation_space
        self.action_space = env.action_space
        ...
        self.observation_in_feature = np.zeros(env.observation_space.shape)
        self.observation_in_features = sum(len(row) for row in self.observation_in_feature)
        self.state_dim = self.observation_in_features + 2
        ...
        self.target_actor = DDPGActor(self.state_dim, self.action_dim).to(device)
        self.target_critic = DDPGCritic(self.state_dim, self.action_dim).to(device)
    ...
    def select_action(self, joint_state_goal, explore=True):
        state = torch.from_numpy(joint_state_goal).float().to(self.device)
        with torch.no_grad():
    ...
    def update_controller(self, offline_replay, online_replay):
        offline_batch_size = int(0.6 * self.batch_size)
        online_batch_size = self.batch_size - offline_batch_size

        if len(offline_replay) < offline_batch_size or len(online_replay) < online_batch_size:
            return

        off_states, off_actions, off_next_states, off_rewards, off_dones = offline_replay.sample(offline_batch_size)
        on_states, on_actions, on_next_states, on_rewards, on_dones = online_replay.sample(online_batch_size)
        ...
        states_var = torch.from_numpy(states).float().to(self.device)
        actions_var = torch.from_numpy(actions).long().to(self.device)
        next_states_var = torch.from_numpy(next_states).float().to(self.device)
        rewards_var = torch.from_numpy(rewards).float().to(self.device).unsqueeze(1)
        dones_var = torch.from_numpy(dones).float().to(self.device).unsqueeze(1)

        action_one_hot = self._one_hot(actions_var)

        with torch.no_grad():
            next_actions = self.target_actor(next_states_var)
            next_q = self.target_critic(next_states_var, next_actions)
            td_target = rewards_var + self.gamma * next_q * (1 - dones_var)

        current_q = self.critic(states_var, action_one_hot)
        critic_loss = torch.mean(F.mse_loss(current_q, td_target))
        self.critic_optimizer.zero_grad()
        critic_loss.backward()
        self.critic_optimizer.step()

        actor_actions = self.actor(states_var)
        actor_q = self.critic(states_var, actor_actions)
        actor_loss = -actor_q.mean()

        if offline_batch_size > 0:
            demo_states = torch.from_numpy(off_states).float().to(self.device)
            demo_actions = torch.from_numpy(off_actions).long().to(self.device)
            demo_one_hot = self._one_hot(demo_actions)
            demo_pred = self.actor(demo_states)

            with torch.no_grad():
                q_demo = self.critic(demo_states, demo_one_hot)
                q_pred = self.critic(demo_states, demo_pred)
                q_filter = (q_demo > q_pred).float()

            bc_loss = (q_filter * ((demo_pred - demo_one_hot) ** 2).sum(dim=1, keepdim=True)).mean()
    ...
    def load(self, path):
        ...
        self.target_actor.load_state_dict(checkpoint.get('target_actor', checkpoint['actor']))
