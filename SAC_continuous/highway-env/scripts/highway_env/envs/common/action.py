...
class ContinuousAction(ActionType):
    ...
    STEERING_RANGE = (-np.pi / 50, np.pi / 50)
...
class DiscreteAction(ContinuousAction):
    """
    An discrete goal space of meta-controlers: lane changes, and cruise control set-point.
    """
    ...
    """A mapping of goal indexes to labels."""
    ...
    """A mapping of longitudinal goal indexes to labels."""
    ...
    """A mapping of lateral goal indexes to labels."""
    def __init__(self,
                 env: 'AbstractEnv',
                 acceleration_range: Optional[Tuple[float, float]] = None,
                 steering_range: Optional[Tuple[float, float]] = None,
        ...
        self.goals = self.GOALS_ALL if longitudinal and lateral \
             else self.GOALS_LONGI if longitudinal \
            else self.GOALS_LAT if lateral \
            else None

        if self.goals is None:
            raise ValueError("At least longitudinal or lateral goal must be included")
        self.goals_indexes = {v: k for k, v in self.goals.items()}

    def goal_space(self) -> spaces.Space:
        return spaces.Discrete(len(self.goals))
    ...
    def act(self, action: int) -> None:
        ...
        '''
    def get_available_goals(self) -> List[int]:
        """
        ...
        for l_index in network.side_lanes(self.controlled_vehicle.lane_index):
            if l_index[2] < self.controlled_vehicle.lane_index[2] \
                    ...
                    and self.lateral:
                goals.append(self.goals_indexes['LANE_LEFT'])
            if l_index[2] > self.controlled_vehicle.lane_index[2] \
                    and network.get_lane(l_index).is_reachable_from(self.controlled_vehicle.position) \
                    and self.lateral:
                goals.append(self.goals_indexes['LANE_RIGHT'])
        if self.controlled_vehicle.speed < self.controlled_vehicle.target_speed and self.longitudinal:
            goals.append(self.goals_indexes['FASTER'])
        if self.controlled_vehicle.speed > 0 and self.longitudinal:
            goals.append(self.goals_indexes['SLOWER'])
        return goals    
    '''
...
class GOAL(DiscreteAction):
    ...
    def __init__(self,
                 ...
                 **kwargs) -> None:
        """
        Create a discrete goal space of meta-goal.
        ...
        self.lateral = lateral
        self.target_speeds = np.array(target_speeds) if target_speeds is not None else MDPVehicle.DEFAULT_TARGET_SPEEDS
        self.goals = self.GOALS_ALL if longitudinal and lateral else None
        #     else self.GOALS_LONGI if longitudinal \
        #    else self.GOALS_LAT if lateral \
