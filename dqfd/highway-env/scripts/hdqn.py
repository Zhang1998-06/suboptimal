    ...
    for i_thousand_episode in range(n_thousand_episode):
        ...
        for i_episode in range(check_frequence):
            ...
            while not done:
                ...
                epsilon = epsilon_schedule.value(total_timestep) if epsilon_schedule is not None else 0.0
                action = agent.select_action(joint_state_goal, epsilon=epsilon)
                next_state, env_reward, done, _ = env.step(action) #high_level_reward,low_level_reward,
                next_state_rule=_["rule_state"] 

                new_reward = env_reward
                ...
                if not done:
                    ...
                    stats.entropy[i_thousand_episode*check_frequence + i_episode]= 0
                    stats.exp_entropy[i_thousand_episode*check_frequence + i_episode]= 0
