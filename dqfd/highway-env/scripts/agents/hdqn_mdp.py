...
class DQfDAgent:
    ...
    def __init__(
        ...
        self.q_network = QValueNet(self.state_dim, self.action_dim).to(device)
        self.target_q_network = QValueNet(self.state_dim, self.action_dim).to(device)
        self.target_q_network.load_state_dict(self.q_network.state_dict())

        self.optimizer = torch.optim.Adam(self.q_network.parameters(), lr=q_lr)
        self.gamma = gamma
        self.tau = tau
        self.device = device
    ...
    def _td_target(self, rewards, next_states, dones):
        with torch.no_grad():
            next_q_values = self.q_network(next_states)
            next_actions = torch.argmax(next_q_values, dim=1, keepdim=True)
            next_q_target = self.target_q_network(next_states).gather(1, next_actions)
    ...
    def update_controller(self, offline_replay, online_replay):
        ...
        actions_var = torch.from_numpy(actions).long().to(self.device).unsqueeze(1)
        ...
        dones_var = torch.from_numpy(dones.astype(np.float32)).float().to(self.device).unsqueeze(1)

        td_target = self._td_target(rewards_var, next_states_var, dones_var)
        q_values = self.q_network(states_var)
        q_action = q_values.gather(1, actions_var)
        td_loss = F.mse_loss(q_action, td_target)

        demo_loss = torch.tensor(0.0, device=self.device)
        if offline_batch_size > 0:
            demo_states_var = torch.from_numpy(off_states).float().to(self.device)
            demo_actions_var = torch.from_numpy(off_actions).long().to(self.device).unsqueeze(1)
            demo_q_values = self.q_network(demo_states_var)
            demo_q_action = demo_q_values.gather(1, demo_actions_var)
            margin = torch.full_like(demo_q_values, self.demo_margin)
            margin.scatter_(1, demo_actions_var, 0.0)
            max_margin_q = (demo_q_values + margin).max(dim=1, keepdim=True)[0]
            demo_loss = torch.mean(F.relu(max_margin_q - demo_q_action))

        total_loss = td_loss + self.demo_weight * demo_loss
