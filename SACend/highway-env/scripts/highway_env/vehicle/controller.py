...
class ControlledVehicle(Vehicle):
    ...
    MAX_STEERING_ANGLE = np.pi / 36  # [rad]
    DELTA_SPEED = 2.5  # [m/s]
    ...
    def __init__(self,
        ...
        self.route = route
        self.goal_speed=None or self.speed
    ...
    def set_goal(self, goal=None):
        self.follow_road()
        if goal== "FASTER":
            self.goal_speed =self.speed + self.DELTA_SPEED
            self.goal_lane_index= self.lane_index
        elif goal=="IDLE":
            ...
            if self.road.network.get_lane(goal_lane_index).is_reachable_from(self.position):
                self.goal_lane_index = goal_lane_index
                #self.goal_speed += 0
                '''
                print(goal)
                print(self.speed)
...
'''
    def act(self, action: Union[dict, str] = None) -> None:
        ...
        '''
        '''
        if should use other thing instead of target lane index and target speed 
