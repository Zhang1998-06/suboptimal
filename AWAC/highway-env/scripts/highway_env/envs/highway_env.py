...
class HighwayEnv(AbstractEnv):
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            ...
            #"reward_distance": 1.5,
            "diff_goal_speed":0,#0.1,
            "diff_goal_lane":0,#0.15,
    ...
    def egovehicleacceleration(self):
        return self.vehicle.action["acceleration"]
    ...
    def crash_reason(self) -> bool:
        stopped= self.vehicle.speed * np.cos(self.vehicle.heading)<=0
        inverse=np.cos(self.vehicle.heading)<=0
        #print(inverse)
        return [self.vehicle.crashed , \
               stopped , inverse , (not self.vehicle.on_road)]
    ...
    def set_goal(self,goal_1=None):
        print(goal_1)
        self.vehicle.set_goal(goal_1) 
    
    def goal_low_action(self):
        #print("check similarity")
        #print(self.vehicle.act_goal())
        
       # print(self.vehicle.action["steering"],self.vehicle.action["acceleration"])
        return self.vehicle.act_goal()
    ...
    def explain_goal(self,goal=None):
        if (goal == "LANE_RIGHT" or goal == "LANE_LEFT") and self.vehicle.target_lane_index==self.vehicle.lane_index:
            goal="IDLE"
        if (goal== "FASTER and LANE_RIGHT" or goal == "FASTER and LANE_LEFT") and self.vehicle.target_lane_index==self.vehicle.lane_index:
            goal="FASTER"
        if (goal == "SLOWER and LANE_LEFT" or goal == "SLOWER and LANE_RIGHT") and self.vehicle.target_lane_index==self.vehicle.lane_index:
            goal="SLOWER"
