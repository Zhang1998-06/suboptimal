...
class Vehicle(RoadObject):
    ...
    def create_random(cls, road: Road,
        ...
        if speed is None:
            if lane.speed_limit is not None:
                speed = road.np_random.uniform(0.7*lane.speed_limit, 0.75*lane.speed_limit)
            else:
        ...
        x0 = np.max([lane.local_coordinates(v.position)[0] for v in road.vehicles]) \
            if len(road.vehicles) else offset
        x0 += offset * road.np_random.uniform(0.9, 1.1)
        v = cls(road, lane.position(x0, 0), lane.heading_at(x0), speed)
        return v
    @classmethod
    def create_trapvehicle(cls, road: Road,
                      speed: float = None,
                      lane_from: Optional[str] = None,
                      lane_to: Optional[str] = None,
                      lane_id: Optional[int] = None,
        ...
        for v in road.vehicles:          
            if v.lane_index[2]==1:
                lane1vehicle.append(v)
            if v.lane_index[2]==0:
                lane0vehicle.append(v)
        # for here we need to know the position of the ego vehicle
        egovehiclex= np.max([lane.local_coordinates(v.position)[0] for v in lane0vehicle])
       
        if _id ==0: 
        
            x0 = np.max([lane.local_coordinates(v.position)[0] for v in lane0vehicle]) \
                if len(lane0vehicle) else egovehiclex
            x0 += 1.3*offset * road.np_random.uniform(0.9, 1.0)
            minspeed= 11#np.min([v.speed for v in lane0vehicle]) * road.np_random.uniform(0.95, 1.0)
            v = cls(road, lane.position(x0, 0), lane.heading_at(x0), minspeed)
        elif _id ==1: 

            x0 = np.max([lane.local_coordinates(v.position)[0] for v in lane1vehicle]) \
                if len(lane1vehicle) else egovehiclex + 0.1*offset * road.np_random.uniform(0.9, 1.0)  
            x0 -= offset * road.np_random.uniform(0.9, 1.0)   
            minspeed= np.min([v.speed for v in lane1vehicle]) * road.np_random.uniform(0.99, 1)\
                if len(lane1vehicle) else defaultspeed* road.np_random.uniform(0.99, 1.0)
            v = cls(road, lane.position(x0, 0), lane.heading_at(x0), minspeed)
        else:
            v=None
        return v
    ...
    @classmethod
    def create_other_traffic(cls, road: Road,
                      speed: float = None,
                      lane_from: Optional[str] = None,
                      lane_to: Optional[str] = None,
                      lane_id: Optional[int] = None,
        ...
        _id = lane_id if lane_id is not None else road.np_random.choice(len(road.network.graph[_from][_to]))
        lane = road.network.get_lane((_from, _to, _id))
        speed = road.np_random.uniform(1*speed, 1.1*speed)
        default_spacing = 12+1.0*speed
        offset = spacing * default_spacing * np.exp(-5 / 40 * len(road.network.graph[_from][_to]))
