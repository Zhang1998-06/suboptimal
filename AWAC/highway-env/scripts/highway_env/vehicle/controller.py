...
class ControlledVehicle(Vehicle):
    ...
    TAU_PURSUIT = 5 * TAU_HEADING  # [s]
    KP_A = 0.6# / TAU_ACC
    KP_HEADING =2 #/ TAU_HEADING
    KP_LATERAL =0.3 #/ TAU_LATERAL  # [1/s]
    MAX_STEERING_ANGLE = np.pi / 50# [rad]
    DELTA_SPEED = 2.5  # [m/s]
    DEFAULT_TARGET_SPEEDS = np.linspace(5, 15, 5)
    def __init__(self,
                 road: Road,
                 position: Vector,
                 heading: float = 0,
        ...
        self.route = route
        #self.goal_speed=goal_speed or self.speed
        #self.goal_lane_index=None
    ...
    def gettargetspeed(self,speed) -> int:
        """
        ...
        return self.DEFAULT_TARGET_SPEEDS[int(np.clip(
            np.round(x * (self.DEFAULT_TARGET_SPEEDS.size - 1)), 0, self.DEFAULT_TARGET_SPEEDS.size - 1))]

    def set_goal(self, goal=None):
        self.follow_road()
        vehiclespeed=self.speed * np.cos(self.heading)
        #print("vehicle_target_speed")
        #print(vehiclespeed)
        if goal== "FASTER":
            self.target_speed = self.gettargetspeed(vehiclespeed+2.5)
            #self.target_lane_index= self.lane_index
        elif goal=="IDLE":
            self.target_speed = self.gettargetspeed(vehiclespeed)
            #self.target_lane_index= self.lane_index
        elif goal == "SLOWER":
            self.target_speed = self.gettargetspeed(vehiclespeed-2.5)
            #self.target_lane_index= self.lane_index
        elif goal == "LANE_RIGHT":

            _from, _to, _id = self.lane_index
            goal_lane_index = _from, _to, np.clip(_id + 1, 0, len(self.road.network.graph[_from][_to]) - 1)
            if self.road.network.get_lane(goal_lane_index).is_reachable_from(self.position):
                self.target_lane_index = goal_lane_index
        elif goal == "LANE_LEFT":
            _from, _to, _id = self.lane_index
            goal_lane_index = _from, _to, np.clip(_id - 1, 0, len(self.road.network.graph[_from][_to]) - 1)# the last one is the total number of the road
            if self.road.network.get_lane(goal_lane_index).is_reachable_from(self.position):
                self.target_lane_index = goal_lane_index
        elif goal== "FASTER and LANE_RIGHT":
            self.target_speed = self.gettargetspeed(vehiclespeed+2.5)
            _from, _to, _id = self.lane_index
            goal_lane_index = _from, _to, np.clip(_id + 1, 0, len(self.road.network.graph[_from][_to]) - 1)
            if self.road.network.get_lane(goal_lane_index).is_reachable_from(self.position):
                self.target_lane_index = goal_lane_index
        elif goal == "FASTER and LANE_LEFT":
            self.target_speed = self.gettargetspeed(vehiclespeed+2.5)
            _from, _to, _id = self.lane_index
            goal_lane_index = _from, _to, np.clip(_id - 1, 0, len(self.road.network.graph[_from][_to]) - 1)# the last one is the total number of the road
            if self.road.network.get_lane(goal_lane_index).is_reachable_from(self.position):
                self.target_lane_index = goal_lane_index
        elif goal == "SLOWER and LANE_RIGHT":
            self.target_speed = self.gettargetspeed(vehiclespeed-2.5)
            _from, _to, _id = self.lane_index
            goal_lane_index = _from, _to, np.clip(_id + 1, 0, len(self.road.network.graph[_from][_to]) - 1)
            if self.road.network.get_lane(goal_lane_index).is_reachable_from(self.position):
                self.target_lane_index = goal_lane_index
        elif goal == "SLOWER and LANE_LEFT":
            self.target_speed = self.gettargetspeed(vehiclespeed-2.5)
            _from, _to, _id = self.lane_index
            goal_lane_index = _from, _to, np.clip(_id - 1, 0, len(self.road.network.graph[_from][_to]) - 1)# the last one is the total number of the road
            if self.road.network.get_lane(goal_lane_index).is_reachable_from(self.position):
    ...
    def act(self, action: Union[dict, str] = None) -> None:  # what is this doing 


        super().act(action)
    def  act_goal(self):
        steering= np.clip(self.steering_control(self.target_lane_index), -self.MAX_STEERING_ANGLE, self.MAX_STEERING_ANGLE)
        acceleration = np.clip(self.speed_control(self.target_speed), -1, 1)
    ...
    def steering_control(self, target_lane_index: LaneIndex) -> float:
        ...
        discretized_angles = [-self.MAX_STEERING_ANGLE, 0, self.MAX_STEERING_ANGLE]

        # Find the closest discretized value
        steering = min(discretized_angles, key=lambda x: abs(steering_angle - x))
  ...
  # check this 
    def speed_control(self, target_speed: float) -> float:
        ...
        """
        # Compute the raw acceleration
        acc = self.KP_A * (target_speed - self.speed)

        # List of discretized acceleration values
        discretized_acc = [-1, 0, 1]

        # Find the closest discretized value
        acc = min(discretized_acc, key=lambda x: abs(acc - x))
