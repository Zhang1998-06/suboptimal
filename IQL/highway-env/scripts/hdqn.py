...
def hdqn_learning(
    ...
    for i_thousand_episode in range(n_thousand_episode):
        ...
        for i_episode in range(check_frequence):
            overall_episode +=1
            episode_idx = i_thousand_episode*check_frequence + i_episode
            episode_length = 0
            ...
            while not done:
                ...
                stats.episode_rewards[episode_idx] += env_reward 
                stats.episode_low_level_reward[episode_idx]+=env_reward
                stats.episode_lengths[episode_idx] = episode_length
                goal_reached=0
                stats.goal_reached_times[episode_idx]+=goal_reached                              
                ...
                if not done:
                    ...
                    stats.trap[episode_idx]=env.out_of_trap()
                    stats.average_speed[episode_idx]= totalspeed/episode_length
                    stats.distance[episode_idx]= totalspeed
                    stats.crash_times[episode_idx]=env.reach_end_crash()
            print("Finished episode {} | reward {:.2f} | length {} | crash {}".format(
                overall_episode,
                stats.episode_rewards[episode_idx],
                stats.episode_lengths[episode_idx],
                stats.crash_times[episode_idx]))
                # Goal Finished
                #这里添加平均最好值
